...
class Trainer(object):
    ...
    def drop_checkpoint(self, opt, epoch, fields, valid_stats,path):
        ...
        torch.save(checkpoint, '%s.pt' % (path))
        decode_cmd = 'python {0}/translate.py -model {1} -src {2} -output {3}' \
                     ' -gpu {4}'.format(
            opt.opennmt_path, '{0}.pt'.format(path), opt.valid_src_raw,
            '{0}_preds'.format(path), opt.gpuid[0])
        evaluate_cmd = "perl {0}/tools/multi-bleu.perl {1} < {2}".format(
            opt.opennmt_path, opt.valid_tgt_raw, '{0}_preds'.format(path))
